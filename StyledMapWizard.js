...
function init() {
  ...
  ukie = new google.maps.LatLngBounds(
    new google.maps.LatLng(49.0, -10.0),
    new google.maps.LatLng(66.0, 2.0)
  );
  aunz = new google.maps.LatLngBounds(
...
function showJson() {
  ...
  for (var i = 0; i < styles.length; i++) {
    ...
    if (styles[i].featureType != 'all') {
      var fixedFeature = styles[i].featureType || 'All';
      jsonStyles[i] += '    "featureType": "' + fixedFeature + '"';
      hasPreviousEntry = true;
    }

    if (styles[i].elementType != 'all') {
      var fixedElement = styles[i].elementType || 'All';

      if (hasPreviousEntry) {
        jsonStyles[i] += ',\n';
      }
      jsonStyles[i] += '    "elementType": "' + fixedElement + '"';
      hasPreviousEntry = true;
    }

    if (styles && styles[i] && styles[i].stylers && styles[i].stylers.length) {
      if (hasPreviousEntry) {
        jsonStyles[i] += ',\n';
      }
...
function loadJson() {
  ...
  var json = document.getElementById('json-editor').value;
  if (json.replace(/\{|\[|\]|\}/g,'').trim() === '') {
    return;
  }

  document.getElementById('mapStyleScrollable').innerHTML = '';

  styles = eval(json);
  mapStyleRenderer.styles = styles;
  mapStyleRenderer.panels = [];

  styles.forEach(function(style, i) {
    if (typeof style.stylers !== 'undefined' && style.stylers.length) {
      loadMapStyle(i);
      setMapStyle();
    }
  });
...
function getStaticMapsURL() {
  ...
  for (var i = 0; i < styles.length; i++) {
    ...
    if (styles[i].featureType != 'all' && typeof styles[i].featureType !== 'undefined') {
      styleRule.push('feature:' + styles[i].featureType);
    }

    if (styles[i].elementType != 'all' && typeof styles[i].elementType !== 'undefined') {
...
MapStyleRenderer.prototype.selectPanel = function(selectedIndex) {
  if (this.selected != null && this.panels[this.selected]) {
...
MapStyleRenderer.prototype.getPanel = function(i) {
  ...
  this.addRow(table, 'featureType', style.featureType || 'All');
  this.addRow(table, 'elementType', style.elementType || 'All');
